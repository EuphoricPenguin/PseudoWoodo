...
## Features
- Dynamic typing - I'm lazy; I'm using language features in JavaScript to save me the trouble of implementing new ones.
- Browser compatibility - The interpreter is written in JS; unless you're using something ancient, it should work fine in-browser.
- Minimalist philosophy - A proof-of-concept language with Pseudocode-like syntax and a barebones interpreter.

## Reserved Names

PseudoWoodo uses several reserved names/keywords, as outlined in the following specification. Reserved names and session calls should be written in lowercase. Reserved names cannot be used as variable names (except for reserved variables) and will result in an error.
...
Variables are defined using the `set` and `as` keywords. Keep in mind that PseudoWoodo does not have block scope; all variables are functionally global.
...
If you wish to change the value of a variable later on, you can use the `as` keyword without `set` to change the value.
...
For variable names, PseudoWoodo uses Lower Kebab Case. Other conventions are syntactically valid, but `lower-kebab-case` is the official recommendation.
...
As an interpreted language, PseudoWoodo is largely sequential in its execution. The exception is its call functionality, which allows the use of conditional and reusable sections of code. Since calls and variable names are tracked separately, a call and a variable can have the same name.
...
Following PseudoWoodo's existing convention of using plain-meaning English keywords, comparison operators are no different.
...
To facilitate more complex logic, standard logic operators are also available. Bitwise operators are not present.
...
PseudoWoodo has several built-in session calls that add essential functionality to the program.
Under the hood, all are passthroughs for native JavaScript functionality.

Since PseudoWoodo doesn't have functions (or function headers), it instead uses reserved variable names for this functionality.
When a session call is invoked, it will read the values of the corresponding reserved variables.

The session call `timeout` provides a synchronous timeout that halts execution for the specified duration. The `timeout` reserved variable is passed a valid JavaScript Number type in whole seconds (not milliseconds).
...
As mentioned previously, conditional statements rely on the call functionality.
...
This will output each array element to the console.
